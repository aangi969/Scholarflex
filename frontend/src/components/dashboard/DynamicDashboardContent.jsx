...
// Apply filtering only if tables exist
if (tables.length > 0) {
...
// 🔥 Fix: If only 1 item OR empty → use fallback
if (pieData.length <= 1) {
  const fallback = getCardData()
  pieData = fallback.pieData
}

const fallback = getCardData()

const finalTables =
  tables && tables.length > 0 && tables.some(t => t.data && t.data.length > 0)
    ? tables
    : fallback.tables
...
setCardData({
  ...
  const tables = (cardData.tables && cardData.tables.length > 0)
  ? cardData.tables
  : fallbackTables
