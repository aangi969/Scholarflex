...
export default function PieChart({ data }) {
  ...
  const segments = data.map((item, index) => {
    ...
    return {
      ...
      path: `M 100 100 L ${x1} ${y1} A ${radius} ${radius} 0 ${largeArcFlag} 1 ${x2} ${y2} Z`,
    }
  })
  ...
  return (
    <div className="flex flex-col lg:flex-row items-center gap-6 sm:gap-8 lg:gap-10">
      ...
      <div className="flex-shrink-0">
        <svg
          viewBox="0 0 200 200"
          className="w-48 h-48 sm:w-56 sm:h-56 lg:w-64 lg:h-64 xl:w-72 xl:h-72 drop-shadow-sm"
        >
      ...
      <div className="flex-1 w-full lg:w-auto space-y-2 sm:space-y-3">
        ...
        {segments.map((segment, index) => (
          <div key={index} className="flex items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              ...
              <span className="text-[10px] sm:text-xs text-gray-500 w-10 sm:w-12 text-right">
                ({segment.percentage}%)
              </span>
            </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs sm:text-sm lg:text-base font-semibold text-[#4C763B]">
            {total}
          </span>
        </div>
